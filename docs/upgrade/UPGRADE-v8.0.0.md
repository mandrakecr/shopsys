...
- change `IvoryCKEditorBundle` into `FOSCKEditorBundle` ([#1072](https://github.com/shopsys/shopsys/pull/1072))
    - replace the registration instance of the bundle in `app/AppKernel`
        ```diff
        ...
        ```
    - replace the name of the `app/config/packages/ivory_ck_editor.yml` into `app/config/packages/fos_ck_editor.yml` and its content
        ```diff
        - ivory_ck_editor:
        + fos_ck_editor:
        ```
    - change the package in `composer.json`
        ...
        ```
    - update all usages of old bundle in
        ...
        - php code like
            ...
            ```
    - launch `php phing composer-dev assets`
